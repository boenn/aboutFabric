...
func (csp *impl) getECKey(ski []byte) (pubKey *sm2.PublicKey, isPriv bool, err error) {
	p11lib := csp.ctx
...
func namedCurveFromOID(oid asn1.ObjectIdentifier) elliptic.Curve {
	...
	case oid.Equal(oidNamedCurveP256_sm2):
		return sm2.P256_sm2()
	}
	return nil
}
...
func (csp *impl) generateECKey(curve asn1.ObjectIdentifier, ephemeral bool) (ski []byte, pubKey *sm2.PublicKey, err error) {
	p11lib := csp.ctx
	session := csp.getSession()
	defer csp.returnSession(session)
...
func (csp *impl) verifyP11SM2(ski []byte, msg []byte, R, S *big.Int, byteSize int) (valid bool, err error) {
	p11lib := csp.ctx
	session := csp.getSession()
	defer csp.returnSession(session)
