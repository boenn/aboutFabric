...
var (
	...
	curveHalfOrders1 map[elliptic.Curve]*big.Int = map[elliptic.Curve]*big.Int{
...
func verifyECDSA(k *ecdsa.PublicKey, signature, digest []byte, opts bccsp.SignerOpts) (valid bool, err error) {
	...
	lowS, err := IsLowSq(k, s)
	if err != nil {
		return false, err
	}

	if !lowS {
		return false, fmt.Errorf("Invalid S. Must be smaller than half the order [%s][%s].", s, curveHalfOrders1[k.Curve])
	}

	return ecdsa.Verify(k, digest, r, s), nil
}

func SignatureToLowSq(k *ecdsa.PublicKey, signature []byte) ([]byte, error) {
	r, s, err := UnmarshalECDSASignature(signature)
	if err != nil {
		return nil, err
	}

	s, modified, err := ToLowSq(k, s)
	if err != nil {
		return nil, err
	}
...
func IsLowSq(k *ecdsa.PublicKey, s *big.Int) (bool, error) {
	halfOrder, ok := curveHalfOrders1[k.Curve]
	if !ok {
...
func ToLowSq(k *ecdsa.PublicKey, s *big.Int) (*big.Int, bool, error) {
	lowS, err := IsLowSq(k, s)
	if err != nil {
