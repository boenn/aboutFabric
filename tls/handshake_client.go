...
func (hs *clientHandshakeState) doFullHandshake() error {
	...
	findCert:
		for i, chain := range c.config.Certificates {
			...
			for j, cert := range chain.Certificate {
				...
				case sm2Avail && x509Cert.PublicKeyAlgorithm == x509.SM2:
				default:
					continue findCert
				}
	...
	if chainToSend != nil {
		...
		case *ecdsa.PublicKey:
			fmt.Println("ecdsa_certificate2")
			signatureType = signatureECDSA
		case *rsa.PublicKey:
			fmt.Println("rsa_certificate2")
			signatureType = signatureRSA
		case *sm2.PublicKey:
			fmt.Println("sm2_certificate2")
			signatureType = signatureSM2SIG
		default:
			c.sendAlert(alertInternalError)
