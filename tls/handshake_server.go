...
func (hs *serverHandshakeState) doFullHandshake() error {
	...
	if len(c.peerCertificates) > 0 {
		...
		if certVerify.hasSignatureAndHash {
			...
			case *sm2.PublicKey:
				signatureAndHash.signature = signatureSM2SIG
			}
		}
		...
		case *sm2.PublicKey:
			if signatureAndHash.signature != signatureSM2SIG {
	...
	for _, supported := range supportedCipherSuites {
		if id == supported {
			...
			if candidate.flags&suiteECDHE != 0 {
				if !hs.ellipticOk {
					fmt.Println("eeccddhhee")
					continue
				}
				if candidate.flags&suiteECDSA != 0 {
					if !hs.ecdsaOk {
						fmt.Println("eeccddssaa")
						continue
					}
			...
			} else if candidate.flags&suiteSM2KEY != 0 {
				if !hs.ellipticOk {
					continue
				}
				if candidate.flags&suiteSM2SIG != 0 {
					if !hs.sm2sigOk {
						fmt.Println("sm2sigOk")
						continue
					}
				} else if !hs.rsaSignOk {
					continue
